...
table_info_df = module_tables_df[module_tables_df['SOURCE_TABLE'] == selected_table].iloc[0]

# Fetch the data for the tab1
tab1_data=fetch_data(selected_table)

# Check if table information exists
if not table_info_df.any():
    st.warning(f"No table information found in Override_Ref for the selected table: {selected_table}.")
    st.stop()

target_table_name = table_info_df['TARGET_TABLE']
